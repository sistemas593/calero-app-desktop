...
public class OrdenComprasServiceImpl {
    ...
    private Page<CpOrdenComprasEntity> getTipoBusquedaPaginado(Long idData, Long idEmpresa,
        ...
        switch (tipoBusqueda) {
            case TODAS -> {
                return ordenComprasRepository.findAllPaginate(idData, idEmpresa, null,
                        filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                        filters.getSecuencial(), null, pageable);
            }
            case SUCURSAL -> {
                if (Objects.nonNull(filters.getSucursal()) && !filters.getSucursal().isEmpty()) {
                    return ordenComprasRepository.findAllPaginate(idData, idEmpresa, filters.getSucursal(),
                            filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                            filters.getSecuencial(), null, pageable);
                } else {
            ...
            case PROPIAS -> {
                return ordenComprasRepository.findAllPaginate(idData, idEmpresa, null,
                        filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                        filters.getSecuencial(), usuario, pageable);
            }
        }
