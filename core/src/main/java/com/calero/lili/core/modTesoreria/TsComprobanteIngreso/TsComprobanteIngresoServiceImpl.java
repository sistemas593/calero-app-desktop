package com.calero.lili.core.modTesoreria.TsComprobanteIngreso;
...
import com.calero.lili.core.modTesoreria.TsComprobanteIngreso.builder.TsComprobanteIngresoBuilder;
import com.calero.lili.core.modTesoreria.TsComprobanteIngreso.dto.ComprobanteIngresoFilterDto;
...
@Slf4j
public class TsComprobanteIngresoServiceImpl {


    private final CnAsientosRepository cnAsientosRepository;
    private final TsComprobanteIngresoBuilder tsComprobanteIngresoBuilder;
    ...
    public ResponseDto create(Long idData, Long idEmpresa, UUID idComprobanteIngreso, RequestCreationComprobanteIngresoDto request, String usuario) {
        ...
        if (existeComprobante.isPresent()) {
            throw new GeneralException(MessageFormat.format("El comprobante ya existe con sucursal: {0}," +
                    ...
                    request.getSucursal(), request.getCodigoSerie(), request.getNumeroComprobante()));
        }
    ...
    public ResponseDto update(Long idData, Long idEmpresa, UUID idComprobante, RequestCreationComprobanteIngresoDto request, String usuario) {
        ...
        CnAsientosEntity entity = cnAsientosRepository.findByIdEntity(idData, idEmpresa, idComprobante, null, null)
        ...
        entity.setNumeroAsiento(request.getNumeroComprobante());
    ...
    public void delete(Long idData, Long idEmpresa, UUID idComprobante, String usuario) {
        CnAsientosEntity existeComprobante = cnAsientosRepository
                .findByIdEntity(idData, idEmpresa, idComprobante, null, null)
    ...
    public ResponseComprobanteIngresoDto findById(Long idData, Long idEmpresa, UUID idComprobante) {
        return tsComprobanteIngresoBuilder.builderResponse(cnAsientosRepository
                .findByIdEntity(idData, idEmpresa, idComprobante, null, null)
                .orElseThrow(() -> new GeneralException(MessageFormat.format("La factura ya existe Secuencia: {0}", idComprobante))));

    }

    public PaginatedDto<ResponseComprobanteIngresoDto> findAllPaginate(Long idData, Long idEmpresa, ComprobanteIngresoFilterDto filter, Pageable pageable) {
        ...
        List<ResponseComprobanteIngresoDto> dtoList = page.stream().map(tsComprobanteIngresoBuilder::builderResponse).toList();
    ...
    public void generateComprobanteIngresoPdf(Long idData, Long idEmpresa, UUID idComprobanteIngreso, HttpServletResponse response) {


        CnAsientosEntity comprobante = cnAsientosRepository
                .findByIdEntity(idData, idEmpresa, idComprobanteIngreso, null, null)
                .orElseThrow(() -> new GeneralException(MessageFormat.format("La factura ya existe Secuencia: {0}", idComprobanteIngreso)));


        try {

            response.setContentType("application/pdf");
            String filename = "Comprobante_Ingreso_ " + LocalDate.now().toString().replace("-", "") + ".pdf";
            ...
            if (Objects.nonNull(comprobante)) {
                ...
                datosEncabezado.addCell(getCell("Comprobante de Ingreso", titleFont, Element.ALIGN_LEFT, Rectangle.NO_BORDER));
                ...
                filaBeneficiario.addCell(getCell("Nombre:", boldFont, Element.ALIGN_LEFT, Rectangle.NO_BORDER));
                filaBeneficiario.addCell(getCell(getNombre(comprobante), normalFont, Element.ALIGN_LEFT, Rectangle.NO_BORDER));
...
*/
    ...
    private String getNombre(CnAsientosEntity comprobante) {

        if (Objects.nonNull(comprobante.getNombre()) && Objects.nonNull(comprobante.getTercero().getTercero())) {
