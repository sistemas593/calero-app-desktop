...
@Service
@AllArgsConstructor
public class LiquidacionesReembolsoPdfXmlServiceImpl {
    ...
    public ArchivoDto findPDFSustentoLiqReembolsoFacturaById(Long idData, Long idEmpresa, UUID id) {
        ...
        CpLiquidacionReembolsoOneProjection entidad = liquidacionReembolsosRepository.findXMLById(idData, idEmpresa, id)
    ...
    private void validarFactura(CpLiquidacionReembolsoOneProjection entidad) {
        if (!entidad.getNumeroAutorizacion().startsWith("01", 8)) {
            throw new GeneralException(MessageFormat.format("El documento con id {0} no corresponde a una factura", entidad.getIdLiquidacionReembolso()));
        }
        validacionComprobante(entidad);
    }

    private void validacionComprobante(CpLiquidacionReembolsoOneProjection entidad) {
