...
public class CotizacionesServiceImpl {
    ...
    private Page<VtCotizacionEntity> getTipoBusquedaPaginado(Long idData, Long idEmpresa, FilterListDto filters,
        ...
        switch (tipoBusqueda) {
            case TODAS -> {
                return cotizacionesRepository.findAllPaginate(idData, idEmpresa, null,
                        filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                        filters.getNumeroIdentificacion(), filters.getSecuencial(), null, pageable);
            }
            case SUCURSAL -> {
                if (Objects.nonNull(filters.getSucursal()) && !filters.getSucursal().isEmpty()) {
                    return cotizacionesRepository.findAllPaginate(idData, idEmpresa, filters.getSucursal(),
                            filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                            filters.getNumeroIdentificacion(), filters.getSecuencial(), null, pageable);
                } else {
            ...
            case PROPIAS -> {
                return cotizacionesRepository.findAllPaginate(idData, idEmpresa, null,
                        filters.getFechaEmisionDesde(), filters.getFechaEmisionHasta(),
                        filters.getNumeroIdentificacion(), filters.getSecuencial(), usuario, pageable);
            }
        }
