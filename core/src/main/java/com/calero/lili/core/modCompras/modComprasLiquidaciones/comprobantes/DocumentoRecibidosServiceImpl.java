    ...
    public CpImpuestosRecibirListCreationResponseDto createFilesLiqReembolso(Long idData, Long idEmpresa,
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                if (!reembolsoRecibidaService.verificarExisteDocumentoElectronicoLiqReembolsoBdd(idData, idEmpresa, model.getNumeroAutorizacion())) {
                    res.setClaveAcceso(model.getNumeroAutorizacion());

                    if (!reembolsoRecibidaService.guardarComprobanteLiqReembolso(idData, idEmpresa, model, usuario)) {
                        res = cpImpuestoRecibirBuilder
                                .builder(nameFile, MensajeComprobante.ERR_LEER_DOCUMENTO_INTERNO, Boolean.FALSE, model.getNumeroAutorizacion());
                    }
                } else {

                    if (reembolsoRecibidaService.validarDocumento(model.getNumeroAutorizacion())) {
                        ...
                        res = cpImpuestoRecibirBuilder.builder(nameFile,
                                MensajeComprobante.ERR_EL_DOCUMENTO_NO_CORRESPONDE, Boolean.FALSE, model.getNumeroAutorizacion());
                    }
